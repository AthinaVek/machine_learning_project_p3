...
void silhouette2(vector< vector<int> > clusters, vector< vector<unsigned short> > centroids, vector< vector<unsigned short> > pVec, int k, int d, ofstream &ofile){
	...
	for (int i=0; i<k; i++){
		...
		for (int j=0; j<clusters[i].size(); j++){
			...
			for (int z=0; z<clusters[i].size(); z++){
				if (j != z)
					a += (int)manhattan_dist2(pVec[clusters[i][j]], pVec[clusters[i][z]], d);
			}
			a = a / clusters[i].size();

			for (int y=0; y<k; y++){
				if (y != i){
					temp = (int)manhattan_dist2(pVec[clusters[i][j]], centroids[y], d);
					if (temp < min){
						min = temp;
						minC = y;
					}
			...
			for (int z=0; z<clusters[minC].size(); z++){
				b += (int)manhattan_dist2(pVec[clusters[i][j]], pVec[clusters[minC][z]], d);
			}
			b = b / clusters[minC].size();
